...
> 构建 coding agent 中的 Discovery fixture 切片。
...
agent 需要具备slash command、markdown skill、条件发现、hook 和命令 attachment能力。本章单独拆出 **条件式 Skill 发现**，让学习者先理解一个边界，再把它组合进完整 runtime。
...
demo 刻意保持小而具体。它会为 条件式 Skill 发现 生成一条可检查 trace，并在无法产出预期 artifact 时失败。
...
可复用产物是 [outputs/03-conditional-skill-discovery.zh-CN.md](../outputs/03-conditional-skill-discovery.zh-CN.md)。
