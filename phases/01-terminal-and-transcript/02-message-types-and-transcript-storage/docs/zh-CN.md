...
> 构建 coding agent 中的 JSON transcript schema 切片。
...
agent 需要具备终端交互、消息持久化、重放、流式输出和诊断信息能力。本章单独拆出 **消息类型与 Transcript 存储**，让学习者先理解一个边界，再把它组合进完整 runtime。
...
demo 刻意保持小而具体。它会为 消息类型与 Transcript 存储 生成一条可检查 trace，并在无法产出预期 artifact 时失败。
...
可复用产物是 [outputs/02-message-types-and-transcript-storage.zh-CN.md](../outputs/02-message-types-and-transcript-storage.zh-CN.md)。
