...
> 构建 coding agent 中的 Command classifier 切片。
...
agent 需要具备权限模式、文件系统作用域、命令风险、用户审批和审计日志能力。本章单独拆出 **Shell 命令风险分类**，让学习者先理解一个边界，再把它组合进完整 runtime。
...
demo 刻意保持小而具体。它会为 Shell 命令风险分类 生成一条可检查 trace，并在无法产出预期 artifact 时失败。
...
可复用产物是 [outputs/03-shell-command-risk-classification.zh-CN.md](../outputs/03-shell-command-risk-classification.zh-CN.md)。
