...
> 构建 coding agent 中的 Token budget report 切片。
...
agent 需要具备上下文预算、attachment、memory 文件、手动压缩、自动压缩和恢复 trace能力。本章单独拆出 **上下文窗口统计**，让学习者先理解一个边界，再把它组合进完整 runtime。
...
demo 刻意保持小而具体。它会为 上下文窗口统计 生成一条可检查 trace，并在无法产出预期 artifact 时失败。
...
可复用产物是 [outputs/01-context-window-accounting.zh-CN.md](../outputs/01-context-window-accounting.zh-CN.md)。
