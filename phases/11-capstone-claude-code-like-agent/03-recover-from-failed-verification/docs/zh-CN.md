...
> 构建 coding agent 中的 Recovery trace 切片。
...
agent 需要具备仓库检查、plan-edit-verify loop、失败恢复、打包、差距分析和发布检查能力。本章单独拆出 **从失败验证中恢复**，让学习者先理解一个边界，再把它组合进完整 runtime。
...
demo 刻意保持小而具体。它会为 从失败验证中恢复 生成一条可检查 trace，并在无法产出预期 artifact 时失败。
...
可复用产物是 [outputs/03-recover-from-failed-verification.zh-CN.md](../outputs/03-recover-from-failed-verification.zh-CN.md)。
